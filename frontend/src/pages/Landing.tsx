...
export default function Landing() {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2.5 text-forest">
            ...
            <div className="leading-tight">
              <div className="font-bold text-ink dark:text-white">CORWADO AgriConnect</div>
              <div className="text-[11px] text-slate-500 dark:text-slate-400">Digital Agriculture Platform</div>
            </div>
          </div>
          <nav className="hidden items-center gap-8 text-sm font-medium text-slate-600 dark:text-slate-400 md:flex">
            <a href="#features" className="hover:text-forest dark:hover:text-leaf">Features</a>
            <a href="#impact" className="hover:text-forest dark:hover:text-leaf">Impact</a>
            <a href="#partners" className="hover:text-forest dark:hover:text-leaf">Partners</a>
          </nav>
      ...
      {/* Hero */}
      <section className="relative flex min-h-[88vh] items-center overflow-x-hidden bg-forest-50 dark:bg-slate-900">
        <div className="relative mx-auto grid w-full max-w-7xl items-center gap-12 px-6 py-16 pb-20 lg:grid-cols-2 lg:gap-16 lg:py-20 lg:pb-24">
          <div className="text-center lg:text-left">
            <span className="inline-flex items-center gap-2 rounded-full border border-forest-100 bg-white px-3 py-1 text-sm font-medium text-forest-700 shadow-soft dark:border-forest-800 dark:bg-slate-800 dark:text-leaf">
              <span className="h-2 w-2 rounded-full bg-leaf" /> Digital extension for South Sudan
            </span>
            <h1 className="mt-6 text-4xl font-extrabold leading-tight text-ink dark:text-white lg:text-5xl xl:text-[3.25rem]">
              Empowering Farmers With <span className="text-forest dark:text-leaf">Knowledge & Markets</span>
            </h1>
            <p className="mx-auto mt-5 max-w-lg text-lg leading-relaxed text-slate-600 dark:text-slate-400 lg:mx-0">
      ...
      {/* Features */}
      <section id="features" className="bg-mist py-20 dark:bg-slate-900">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold text-ink dark:text-white">Everything an extension program needs</h2>
            <p className="mt-3 text-slate-600 dark:text-slate-400">A complete toolkit for digital agricultural extension, market access and farmer engagement.</p>
          </div>
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl bg-white p-6 shadow-soft transition hover:shadow-card dark:border dark:border-slate-800 dark:bg-slate-950">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-forest-50 text-forest dark:bg-forest-800/40 dark:text-leaf">
                  <f.icon size={22} />
                </div>
                <h3 className="mt-4 font-semibold text-ink dark:text-white">{f.title}</h3>
      ...
      {/* Impact */}
      <section id="impact" className="py-20 dark:bg-slate-950">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 lg:grid-cols-2">
          <div>
            <h2 className="text-3xl font-bold text-ink dark:text-white">Built for real impact in the field</h2>
            ...
            <ul className="mt-6 space-y-3">
              {[
                ...
                <li key={t} className="flex items-start gap-3">
                  <CheckCircle2 className="mt-0.5 shrink-0 text-forest dark:text-leaf" size={20} />
                  <span className="text-slate-700 dark:text-slate-300">{t}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl bg-forest-800 p-8 text-white shadow-card">
      ...
      {/* Partners */}
      <section id="partners" className="border-t border-slate-100 bg-mist py-14 dark:border-slate-800 dark:bg-slate-900">
        <div className="mx-auto max-w-7xl px-6 text-center">
          <p className="text-sm font-medium uppercase tracking-wide text-slate-400">In partnership with</p>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
            {PARTNERS.map((p) => (
              <span key={p} className="text-lg font-semibold text-slate-500 dark:text-slate-400">{p}</span>
            ))}
          </div>
        </div>
      </section>

      <footer className="bg-forest-800 text-forest-50/80">
